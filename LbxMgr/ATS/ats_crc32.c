 ...
 *  PKZIP, and ZMODEM.  The ats_compute_crc32_buffer() can also be detached and
 *  used independently.
 *
...
static unsigned long ats_compute_crc32_buffer( unsigned long inCrc32, const void *buf,
                                       size_t bufLen );
*/
...
#   define CRC_BUFFER_SIZE  8192
    ...
    *crc32_out = 0;
    while (itr_chunk_count <= chunk_count)
    {
        ...
        *crc32_out = ats_compute_crc32(*crc32_out, buf, chunk_size);
...
#   define CRC_BUFFER_SIZE  8192
    ...
    fseek(stream_file_in, 0, SEEK_SET);

    /** accumulate crc32 from file **/
    *crc32_out = 0;
    while (1)
    {
        bufLen = fread( buf, 1, CRC_BUFFER_SIZE, stream_file_in );
        if (bufLen == 0)
        {
            if (ferror(stream_file_in))
            {
        ...
        *crc32_out = ats_compute_crc32(*crc32_out, buf, bufLen);
    }
...
/*----------------------------------------------------------------------------*\
 *  NAME:
 *     ats_compute_crc32_buffer() - computes the CRC-32 value of a memory buffer
 *  DESCRIPTION:
